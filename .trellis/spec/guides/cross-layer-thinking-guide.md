...
- [ ] **Reminders are consolidated per user**: `process_reminders` aggregates all due subs into one `render(locale, items)` call, then sends one message per channel (not per subscription). `render` takes `list[tuple[Subscription, int]]` pre-sorted by days ascending; single-sub degrades to `subject_single`.
- [ ] Never log credentials — log channel name + user id + sub count (see [Logging Guidelines](../backend/logging-guidelines.md)).
