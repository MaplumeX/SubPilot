...
- **Third-party credentials** — SMTP passwords, Telegram bot tokens, API keys stored on users. When a send fails, log the channel name + user id + sub count (`channel.name`, `user.id`, `sub_count`) — never the credential. `logger.exception("Failed sending %s reminder for user %s (%d subscriptions)", channel.name, user.id, len(due_items))` is the right shape. (Reminders are consolidated into one summary message per channel per user, so there is no single `sub.id` to log.)
