...
- **Form state not resetting on reopen** — a form component reused across dialog opens must re-sync its `useState` fields when `open` becomes `true`, because `useState` initialisers only run on mount and a fixed `key` (e.g. `key="app-create"` in `AppLayout`) or unchanged `key` (e.g. `key={editing?.id ?? "create"}` in `SubscriptionsPage` when creating twice in a row) means the instance is not remounted. `SubscriptionForm` handles this internally with a `useEffect([open, subscription])` that resets all form + auxiliary state on open; do not rely on external `key` changes for reset.
...
- Currency amounts: use `formatCurrency(amount, currency, locale)` from `@/lib/currencies` for any user-facing original-currency price (subscription cards/tables, dashboard due-soon, calendar popover). It wraps `Intl.NumberFormat(locale, { style: "currency", currency })` so the browser ICU library handles symbol + locale disambiguation (e.g. `en` → `$9.99` for USD, `CA$9.99` for CAD; `zh-CN` → `US$9.99`, `CA$9.99`) and falls back to `<code> <amount>` on construction failure. Do NOT hardcode `{currency} {price.toFixed(2)}` — that loses the symbol and locale prefix. Converted/base-currency amounts already use `Intl.NumberFormat` directly and do not need the wrapper.
